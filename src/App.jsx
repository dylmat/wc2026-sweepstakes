...
const WORKER_BASE = 'YOUR_WORKER_URL'; // e.g. https://wc-proxy.yourname.workers.dev
...
function useWorldCupData() {
  ...
  return { standings, matches, odds, loading, error, lastUpdated, usingDemoMode };
}
...
function parseStandings(data) {
  ...
  (totalStage.table || []).forEach(row => {
    ...
    byTeam[name] = {
      team: name,
      mp: row.playedGames, w: row.won, d: row.draw, l: row.lost,
  ...
  Object.entries(GROUPS).forEach(([grp, teams]) => {
    result[grp] = teams
      .map(t => byTeam[t] || { team: t, mp: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 })
...
function parseOdds(data) {
  ...
  (data || []).forEach(game => {
    ...
    if (!h2h) return;
    const outcomes = {};
    h2h.outcomes?.forEach(o => { outcomes[normalizeTeam(o.name)] = o.price; });
    map[key] = {
...
function computeGroupStandings(apiStandings, scheduleMatches) {
  if (apiStandings && Object.keys(apiStandings).length > 0) {
    ...
    Object.entries(GROUPS).forEach(([grp, teams]) => {
      if (apiStandings[grp]?.length > 0) {
        ...
        result[grp] = teams.map(t => ({ team: t, mp: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 }));
      }
    });
    return result;
  }
  ...
  scheduleMatches.filter(m => m.score && m.group).forEach(m => {
    const [g1, g2] = m.score.ft;
    const t1 = m.team1, t2 = m.team2, grp = m.group;
    const s1 = standings[grp]?.[t1], s2 = standings[grp]?.[t2];
    if (!s1 || !s2) return;
    s1.mp++; s2.mp++; s1.gf += g1; s1.ga += g2; s2.gf += g2; s2.ga += g1;
...
function computeKnockoutResults(matches) {
  ...
  matches.filter(m => m.score).forEach(m => {
    ...
    const isKnockout = ['ROUND_OF_32','LAST_32','ROUND_OF_16','LAST_16','QUARTER','SEMI','FINAL'].some(k => stage.includes(k));
    if (!isKnockout) return;
    const [g1, g2] = m.score.ft;
    let winner = null, loser = null;
    if (g1 > g2) { winner = m.team1; loser = m.team2; }
    else if (g2 > g1) { winner = m.team2; loser = m.team1; }
    if (!winner) return;
    let rk = 'Round of 32';
    ...
    else if (stage.includes('THIRD') || stage.includes('3RD')) rk = '3rd Place';
...
function computeSweepstakesPoints(knockoutResults) {
  const pp = {};
  PLAYERS.forEach(p => { pp[p.id] = { total: 0, breakdown: {} }; });
...
function MatchCard({ match, oddsMap }) {
  ...
  const timeLabel = isToday(date) ? `Today ${toMelbourneTimeOnly(date)}`
    : isTomorrow(date) ? `Tomorrow ${toMelbourneTimeOnly(date)}`
    : toMelbourne(date);

  const stageLabel = group ? `Group ${group}` : round?.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || '';
  ...
  return (
    <div className={`match-card ${isLive ? 'live' : ''} ${isFinished ? 'finished' : ''}`}>
      ...
      <div className="mc-teams">
        {/* Team 1 */}
        <div className="mc-team" style={{ borderBottom: p1 ? `2px solid ${p1.color}` : 'none' }}>
        ...
        <div className="mc-score-wrap">
          {isFinished || isLive ? (
            <div className="mc-score">
              <span className={score?.ft[0] > score?.ft[1] ? 'mc-winner' : ''}>{score?.ft[0] ?? '–'}</span>
              <span className="mc-sep">:</span>
              <span className={score?.ft[1] > score?.ft[0] ? 'mc-winner' : ''}>{score?.ft[1] ?? '–'}</span>
            </div>
          ) : (
        ...
        {/* Team 2 */}
        <div className="mc-team mc-team-right" style={{ borderBottom: p2 ? `2px solid ${p2.color}` : 'none' }}>
      ...
      {matchOdds && isScheduled && (
        <div className="mc-odds">
          <div className="mc-odd" style={{ color: p1?.color || 'var(--text-secondary)' }}>
          ...
          <div className="mc-odd" style={{ color: p2?.color || 'var(--text-secondary)' }}>
...
function UpcomingSection({ odds, usingDemoMode }) {
  ...
  relevant.forEach(m => {
    ...
    if (!byDay[day]) byDay[day] = { label: toMelbourneDateOnly(m.date), matches: [] };
    ...
    <section className="section">
      ...
      {usingDemoMode && (
        <div className="demo-banner">
          📋 <strong>Schedule mode</strong> — Deploy the proxy Worker to enable live scores &amp; odds (see README).
        </div>
      )}
      {days.map(({ label, matches }) => (
        <div key={label} className="day-group">
          <div className="day-label">
            {isToday(matches[0].date) ? '📍 Today' : isTomorrow(matches[0].date) ? '⏭ Tomorrow' : `📅 ${label}`}
          </div>
          <div className="match-cards-row">
            {matches.map(m => <MatchCard key={m.id} match={m} oddsMap={odds} />)}
          </div>
        </div>
      ))}
    </section>
...
function LeaderboardChart({ playerPoints }) {
  const data = PLAYERS.map(p => ({
    ...p, total: playerPoints[p.id]?.total || 0, breakdown: playerPoints[p.id]?.breakdown || {},
...
export default function App() {
  const { standings: apiStandings, matches, odds, loading, error, lastUpdated, usingDemoMode } = useWorldCupData();
  const [activeTab, setActiveTab] = useState('schedule');

  const groupStandings  = useMemo(() => computeGroupStandings(apiStandings, SCHEDULE), [apiStandings]);
  ...
  const isLive = matches.some(m => m.status === 'IN_PLAY' || m.status === 'LIVE');
    ...
    <div className="app">
      <header className="header">
        <div className="header-inner">
          ...
          <div className="header-meta">
            ...
            {usingDemoMode && <span className="status-pill demo">📋 Add API key for live scores</span>}
      ...
      <nav className="tab-nav">
        {['schedule','groups','knockout'].map(tab => (
          <button key={tab} className={`tab ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab)}>
            {tab === 'schedule' ? 'Schedule' : tab === 'groups' ? 'Groups' : 'Knockout'}
          </button>
        ))}
      </nav>
      ...
      <div className="body">
        <main className="main-content">
          {activeTab === 'schedule'  && <UpcomingSection odds={odds} usingDemoMode={usingDemoMode} />}
          {activeTab === 'groups'    && <GroupsSection standings={groupStandings} />}
      ...
      <footer className="footer">
        Scores: <a href="https://www.football-data.org" target="_blank" rel="noreferrer">football-data.org</a>
        · Odds: <a href="https://the-odds-api.com" target="_blank" rel="noreferrer">The Odds API</a>
        · Refreshes on load
      </footer>
    </div>
  );
